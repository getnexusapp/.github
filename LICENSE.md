# License

This repository contains documentation, release notes, and community
content for Nexus — **not** the Nexus application source code.

Unless a file states otherwise, content in this repository is licensed
under [Creative Commons Attribution 4.0 (CC BY 4.0)](https://creativecommons.org/licenses/by/4.0/):
you're free to share and adapt it, with attribution to Nexus.

This license does **not** extend to the Nexus application itself, its
source code, or its assets (logo, icons, product name), which remain
proprietary. See [getnexus.app/terms](https://getnexus.app/terms) for
the application's End User License Agreement.
